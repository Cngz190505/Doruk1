...
# Kısa süreli bellek: aynı gün içinde aynı TJK/Nesine sayfasını tekrar tekrar
# dışarıdan çekmeyelim. Bu özellikle telefon açılışını hızlandırır.
_RESPONSE_CACHE = {}

def cached_fetch(url, ttl=15):
    now = time.time()
    hit = _RESPONSE_CACHE.get(url)
    if hit and now - hit[0] < ttl:
        return hit[1]
    r = fetch_upstream(url)
    _RESPONSE_CACHE[url] = (now, r)
    # Belleği sınırsız büyütme.
    if len(_RESPONSE_CACHE) > 80:
...
def nesine_races():
    requested = request.args.get("url", "").strip()
    urls = []
    if requested:
        urls.append(requested)
    urls.extend([
        "https://www.nesine.com/at-yarisi",
        "https://www.nesine.com/at-yarisi/tjk-sabit-ihtimalli-at-yarisi-bulteni",
    ])
    urls = list(dict.fromkeys(urls))
    parsed_all = {"horses": [], "pairs": {"ikili": [], "sirali_ikili": [], "sirali_uclu": []}}
    seen_horses = set()
    seen_pairs = set()
    statuses = []
    ...
    for url in urls:
        if not allowed_upstream(url):
            continue
        try:
            r = cached_fetch(url, ttl=5)
            statuses.append(r.status_code)
            parsed = parse_nesine_odds(r.text)
            for h in parsed.get("horses", []):
                key = (h.get("no"), h.get("isim"))
                if key not in seen_horses:
                    seen_horses.add(key)
                    parsed_all["horses"].append(h)
            for kind, rows in parsed.get("pairs", {}).items():
                for row in rows:
                    key = (kind, row.get("kombinasyon"), row.get("oran"))
                    if key not in seen_pairs:
                        seen_pairs.add(key)
                        parsed_all["pairs"].setdefault(kind, []).append(row)
        except requests.RequestException:
            continue

    parsed_all["parsed"] = bool(parsed_all["horses"] or any(parsed_all["pairs"].values()))
    parsed_all["horse_count"] = len(parsed_all["horses"])
    parsed_all["pair_count"] = sum(len(v) for v in parsed_all["pairs"].values())
    return jsonify({
        "ok": True,
        "source": "nesine-public",
        "status_codes": statuses,
        "odds": parsed_all,
        "note": "Yalnızca kamuya açık sayfalarda dönen içerik ayrıştırılır; giriş/protected API kullanılmaz."
    })
