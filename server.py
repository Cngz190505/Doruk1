...
ALLOWED_HOSTS = {"www.tjk.org", "tjk.org", "www.nesine.com", "nesine.com"}
...
def allowed_url(url):
    ...
    if host in {"www.tjk.org", "tjk.org"}:
        return "gunlukyarisprogrami" in path

    if host in {"www.nesine.com", "nesine.com"}:
...
def proxy():
    ...
    # Browser-side DOMParser needs the actual HTML response.
...
def nesine_races():
    ...
    return jsonify({
        ...
        "bytes": len(r.content),
        "note": "İlk HTML canlı oranları içermiyorsa canlı oran akışı ayrıca bağlanmalıdır."
    })
